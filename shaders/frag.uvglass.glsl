...
struct Material
{
	float diffuse; // diffuse reflection color
	float specular; // specular reflection color
	float ambient; // ambient reflection color
...
void main()
{
	...
	light = light * material.ambient + diffuse * material.diffuse;

	light = light * texel.rgb;// * texel.a;
	...
	light = material.baseIntensity * light +
			material.reflectionIntensity * (reflectLight + material.specular * specular) +
